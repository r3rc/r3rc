...
$TemplatesDir = Join-Path (Get-AgentsDir) 'skills/_shared/sdd/templates'

# Mechanical scaffold/archive for the r3 SDD convention (engine-free). Operates on the
# openspec/ tree in the current project (CWD). Override the target with SDD_OPENSPEC_DIR.
$OpenspecDir = if ($env:SDD_OPENSPEC_DIR) { $env:SDD_OPENSPEC_DIR } else { Join-Path $PWD.Path 'openspec' }
...
function Invoke-InitCommand {
    Assert-Templates
    New-Item -ItemType Directory -Path (Join-Path $OpenspecDir 'specs') -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $OpenspecDir 'changes/archive') -Force | Out-Null
    $config = Join-Path $OpenspecDir 'config.yaml'
    if (Test-Path -LiteralPath $config) {
        Write-Warn "config.yaml already exists — skipped"
    }
    else {
        Copy-Item -LiteralPath (Join-Path $TemplatesDir 'config.yaml') -Destination $config
        Write-Ok "created: openspec/config.yaml"
    }
    Write-Success "initialized openspec/ at $OpenspecDir"
}
...
function Invoke-NewCommand {
    ...
    Assert-Slug $slug
    Assert-Templates
    if (-not (Test-Path -LiteralPath (Join-Path $OpenspecDir 'changes') -PathType Container)) {
        Die "no openspec/changes — run: sdd.ps1 init"
    }
    $dir = Join-Path $OpenspecDir "changes/$slug"
    ...
    Write-Ok "scaffolded empty change: openspec/changes/$slug/ (with specs/)"
...
function Invoke-ArchiveCommand {
    ...
    $src = Join-Path $OpenspecDir "changes/$slug"
    if (-not (Test-Path -LiteralPath $src -PathType Container)) { Die "change '$slug' not found at $src" }
    $archiveRoot = Join-Path $OpenspecDir 'changes/archive'
...
function Invoke-ListCommand {
    $cdir = Join-Path $OpenspecDir 'changes'
    if (-not (Test-Path -LiteralPath $cdir -PathType Container)) {
        Die "no openspec/changes at $cdir — run: sdd.ps1 init"
    }
    $found = $false
...
switch ($Command) {
    ...
    default {
        Usage "sdd.ps1 <init | new <slug> | archive <slug> | list>   (targets `$PWD/openspec; override with SDD_OPENSPEC_DIR)"
    }
}
