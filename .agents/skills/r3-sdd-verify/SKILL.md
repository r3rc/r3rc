...
infers coverage). The spec-format self-check rules are in the auto-loaded rule `sdd-spec-format`.
...
Read the change's `tasks.md`, `specs/<capability>/spec.md`, and `design.md` (if present).

### Step 3 — Check three dimensions

- **Completeness** — _Tasks_: parse `- [ ]` vs `- [x]`; each incomplete task → CRITICAL. _Spec coverage_: for each
  `### Requirement:`, search the codebase for evidence it is implemented; if missing → CRITICAL.
