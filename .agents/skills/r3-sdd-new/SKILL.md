...
# r3-sdd-new — scaffold a change, one artifact at a time
...
Creates the empty change folder `openspec/changes/<slug>/` (with a `specs/` subdir). No artifact content is written — that happens in `r3-sdd-continue` / `r3-sdd-propose`.

## Constraints

- Do NOT fill any artifact here — only scaffold and show the first template.
- One change = one kebab-case folder under `openspec/changes/`.
